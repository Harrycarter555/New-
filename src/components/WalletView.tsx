...
import { userService } from './AdminPanel/firebaseService';
...
const WalletView: React.FC<WalletViewProps> = ({
  ...
  // ✅ Withdrawal request using service
  ...
  // ✅ Viral submission using service
  const handleViralSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      // Use userService for viral submission
      // (You'll need to add this method to userService)
    ...
    <div className="space-y-10 pb-40 animate-slide">
      ...
      {walletTab === 'viral' && (
        <ViralTab 
          ...
          selectedCampaignForViral={selectedCampaignForViral}
          setSelectedCampaignForViral={setSelectedCampaignForViral}
          viralLink={viralLink}
          setViralLink={setViralLink}
...
const TransactionsTab = ({ 
  userPayouts, userSubmissions, withdrawAmount, setWithdrawAmount, 
  handleWithdrawal, isSubmitting, formatCurrency, formatDate 
}: any) => (
        ...
        <>
          {userPayouts.map((payout: any) => (
            <TransactionItem 
              ...
              details={`${payout.method} • ${payout.status}`}
          ...
          {userSubmissions.map((sub: any) => (
            <TransactionItem 
              ...
              date={formatDate(sub.timestamp)}
              details={`${sub.status} • ${sub.isViralBonus ? 'Viral Bonus' : 'Basic'}`}
              amount={formatCurrency(sub.rewardAmount)}
              status={sub.status}
            />
          ))}
        </>
...
const TransactionItem = ({ title, date, details, amount, status }: any) => (
  <div className="flex justify-between items-center p-6 bg-white/5 rounded-3xl border border-white/5 animate-slide">
    <div>
      <p className="text-[10px] font-black text-white italic uppercase leading-none">
        {title}
      </p>
      <p className="text-[8px] font-bold text-slate-600 mt-1">{date}</p>
      <p className="text-[8px] font-bold text-slate-400 mt-1">{details}</p>
    </div>
...
const InboxTab = ({ userBroadcasts, formatDate }: any) => (
...
const PaymentTab = ({ paymentSettings, setPaymentSettings, handleUpdatePayment }: any) => (
...
const ViralTab = ({
  userCampaigns, selectedCampaignForViral, setSelectedCampaignForViral,
  viralLink, setViralLink, handleViralSubmit, pendingSubmissions,
